...
class APITest(unittest.TestCase):
    ...
    def test_url_encode_str(self):
        params = {"aaa": "bbb", "bbb": "ccc", 1: "ddd"}
        self.assertEqual(self.api.url_encode(params), "?aaa=bbb&bbb=ccc&1=ddd")

    def test_url_encode_list(self):
        params = {"aaa": ["bbb", "ccc", "ddd"], "eee": [1, 2, 3], 1: ["fff", 5, 7]}
        self.assertEqual(self.api.url_encode(params), "?aaa=bbb&aaa=ccc&aaa=ddd&eee=1&eee=2&eee=3&1=fff&1=5&1=7")

    def test_url_encode_list_str(self):
        params = {"aaa": "bbb", "bbb": "ccc", 1: ["eee", 123, "fff"]}
        self.assertEqual(self.api.url_encode(params), "?aaa=bbb&bbb=ccc&1=eee&1=123&1=fff")

    def test_url_encode_escape_chr(self):
        params = {"user": "JoshuaL&page=1", "page": 1}
        self.assertEqual(self.api.url_encode(params), "?user=JoshuaL%26page%3D1&page=1")
        params = {"user": "JoshuaL#", "page": 1}
        self.assertEqual(self.api.url_encode(params), "?user=JoshuaL%23&page=1")

    def test_url_encode_none(self):
        params = {"user": "JoshuaL", "page": None}
        self.assertEqual(self.api.url_encode(params), "?user=JoshuaL")
