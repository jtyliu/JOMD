...
class ParseUser:
    ...
    @staticmethod
    async def inits(objs):
        await asyncio.gather(*[ParseUser.init(obj) for obj in objs])
...
class ParseSubmission:
                ...
                async with lock['language']:
                    ...
                    for language in api.data.objects:
                        if language.key not in languages:
                            ...
                            session.add(languages[language.key])
                    session.commit()
            if 'language' in lock:
                async with lock['language']:
        ...
        if hasattr(obj, 'problem'):
            ...
            if obj.problem not in problems and obj.problem not in lock:
                ...
                async with lock[obj.problem]:
                    ...
                    session.add(problems[obj.problem])
                    session.commit()
    ...
    @staticmethod
    async def inits(objs):
        await asyncio.gather(*[ParseSubmission.init(obj) for obj in objs])
