...
async def on_error(event: lightbulb.CommandErrorEvent) -> None:
    if isinstance(event.exception, lightbulb.NotEnoughArguments):
        await event.context.respond(f'Not enough arguments ({event.exception})')
    else:
        trace = ''.join(traceback.format_exception(None, event.exception, event.exception.__traceback__))
        await event.context.respond('''Unhandled exception ({0})
Backtrace:'''.format(event.exception), 
            attachment=hikari.Bytes(trace, "traceback.txt")) # Truncate or face error
...
async def force(ctx: lightbulb.Context) -> None:
    if ctx.options.type.lower() == 'contest':
        ...
        if q.count() == 0:
            await ctx.respond(f'There is no contests with the key {ctx.options.key} '
    ...
    if ctx.options.type.lower() == 'problem':
        ...
        if q.count() == 0:
            await ctx.respond(f'There is no problems with the key {ctx.options.key} '
